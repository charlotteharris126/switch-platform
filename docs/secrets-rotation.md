...
| `iris_writer` password | LastPass: `Supabase — iris_writer password` | Future `iris-daily-flags` Edge Function (stage 2 of the new ads dashboard); INSERT-only on `ads_switchable.iris_flags`, SELECT on a small allowlist of source tables | 2026-05-03 | 2027-05-03 | Created in migration 0056. Rotate via `ALTER ROLE iris_writer WITH PASSWORD ...` in a new migration; also update Edge Function secret once stage 2 ships and the function is wired to use this role |
...
| `PENDING_UPDATE_SECRET` | Supabase Edge Functions → Manage secrets | HMAC signing key for AI-suggestion approval email tokens (Channel B of sheet→DB mirror); 7-day token TTL | Pending Phase 2 activation | 1 year after first set | Generated via `openssl rand -hex 32`. Never in iCloud-synced files. Blast radius on leak: attacker could mint approve/reject/override links for any pending suggestion in `crm.pending_updates` — meaningful at scale, low impact at pilot volume. Used by `sheet-edit-mirror` (signs) + `pending-update-confirm` (verifies). |
| `ANTHROPIC_API_KEY` | Supabase Edge Functions → Manage secrets; master in Anthropic Console → API Keys | Claude API access for `sheet-edit-mirror` Channel B (interpret provider notes) | Pending Phase 2 activation | 1 year after first set | Blast radius: AI calls only — no data read/write. Rotate via Anthropic console, replace Supabase secret, revoke old key. Set monthly spend alarm in Anthropic console as belt-and-braces. Rotated immediately on any suspected leak. |
| `META_ACCESS_TOKEN` | Supabase Edge Functions → Manage secrets; master in Meta Business Manager → System users → Access tokens | `meta-ads-ingest` Edge Function (daily pull of per-ad spend, impressions, leads, CTR into `ads_switchable.meta_daily`) | Set Apr 2026 (exact date pending; was set during Session 22 ingest build) | 60 days before token expiry; long-lived system user tokens are valid 60+ days from issue, Meta requires renewal before expiry or daily ingest 401s | Blast radius: ad-account read/write per `ads_read` + `ads_management` scopes. No PII access. Rotate via Meta Business Manager → System user → Generate new token, replace Supabase secret, no need to revoke old (Meta auto-expires). Set a calendar reminder 7 days before expiry. |
...
| 2026-05-03 | Added `META_ACCESS_TOKEN` row (used by `meta-ads-ingest` daily cron). Token was set during Session 22 ingest build but never logged here. Carryover housekeeping cleared. Tremendous secrets (TREMENDOUS_API_KEY, TREMENDOUS_PRODUCT_ID_AMAZON_UK) NOT added — Mira reverted the Tremendous-from-launch decision the same morning; manual Amazon e-gift fulfilment for v1, Tremendous deferred until 20+ refs/month sustained. Re-evaluate when that trigger fires. |
| 2026-05-03 | Added `iris_writer` Postgres role row. Created in migration 0056 (Iris stage 1a). Will become active when stage 2 (`iris-daily-flags` Edge Function) ships and is wired to authenticate as this role. Annual rotation cadence per default. |
