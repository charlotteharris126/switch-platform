...
2. **PRETTY LINKS — design LOCKED, build it.** Audit result: the ONLY ugly user-facing link is the **fastrack URL** (`switchable.org.uk/funded/thank-you/?ref=<uuid>&course=<slug>&m=1`), used in the fastrack-link SMS + fastrack emails (`SW_FASTRACK_URL`). Referral (`/refer/<code>`) + portal (`app.switchleads.co.uk/leads/<id>`) are already clean — leave them.
   ...
   - **Build split:** Platform owns a `short_links` mapping (code → the fastrack target: submission client_nonce + course + m, or the full URL) + code generation on collision-retry + a lookup EF the site calls. Change `buildFastrackUrl` (route-lead.ts) + `buildFastrackUrlForSms` (sms-utility.ts) to mint+store a code and emit `/go/<code>` instead of the long URL. **Site (Mable):** build the `/go/<code>` route on switchable.org.uk that calls the platform lookup and 302-redirects to the full thank-you URL; rate-limit it (unguessable-code protection). Do NOT flip the platform emit until the site route is live, or links 404.
   - **Cross-project:** switchable/site (Mable — the `/go/` route). Email deep-links auto-fix once `SW_FASTRACK_URL` emits the short form.
