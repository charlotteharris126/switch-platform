...
1. **Investigate the live P2.3 drift signal.** First flag Iris produced shows real Meta-vs-DB lead drift (33% / 50% over two consecutive days). Possible causes: CAPI deduplication, slow-page pixel misses, browser blockers. Worth diagnosing before stages 3-4 ship and surface the flag prominently. Could be a Mable (switchable/site) issue if pixel placement on slow pages.
...
- **Folder:** `platform/`
- **First task:** Investigate the live P2.3 pixel/CAPI drift signal. Pull the actual lead rows for 2026-05-01 and 2026-05-02, cross-check against Stape's CAPI dashboard, identify whether the gap is dedup-related or pixel-fire-related. If switchable/site work is needed, push to Mable.
- **Cross-project:** This session's main outcome (Iris stage 2 live, weekly markdown briefs killed) needs flagging in `switchable/ads/` so Iris's persona understands she no longer runs the daily checks. Push added in step 5 of this `/handoff`.
