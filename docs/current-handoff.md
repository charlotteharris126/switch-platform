...
2. **No-match Brevo upsert (3-state build) in `netlify-lead-router`.** Scope locked with owner 2026-04-29: 3-state `SW_MATCH_STATUS` (matched / pending / no_match), self-funded matrix-by-courseId secondary index, enrichment overwrite confirmed. Full spec in [`platform/docs/no-match-brevo-build.md`](no-match-brevo-build.md) — ~2-hour single-session build covering site script + Edge Function helper + branching + 8-test pass. Tonight's U1+SF2 launch runs on the existing routed-lead path and does NOT block on this.
