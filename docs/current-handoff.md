...
- **(Labs session 9 push):** Gaply calculator at `/models/gaply-calculator.html` reworked (4-step funnel split + Live Test B benchmark preset, 2 commits pushed, live). New Gaply Test A tool live at `/gaply/business/`. `/admin/labs` funnel table relabelled (stale £17 columns to Price clicks / Run to price / Price to email, "Signups" to "Emails", swapped View to £17 for View to Run, plain-English subtitle) — committed + pushed. See Next steps 1 for the remaining split task.

## Next steps
1. **Split Gaply test_a/test_b in /admin/labs** (pushed from labs session 9, Work Hub `platform`). Gaply now runs two smoke tests under `tool='gaply'`, separated only by a `test` payload tag: `test_b` (founders, `/gaply/`) and `test_a` (business owners, `/gaply/business/`). Funnel + targeting aggregate both. Split them so each reads separately. (The stale "£17" column labels were already fixed this session and a View → Run column added; only the split remains.)
