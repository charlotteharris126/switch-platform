...
1. **[PUSHED FROM switchable/social Session 1, 2026-05-02] Scope Session H: Switchable brand toggle in `/social` admin.** Extend `social.drafts` schema with brand + Meta channel discriminators, build Meta Graph API publishing Edge Function, activate brand toggle in `/social?brand=switchable` admin module. Same architecture as the SwitchLeads LinkedIn pipeline. Build effort: ~1 platform session. External wait: Meta App approval (run in parallel with build). Full spec to add to `docs/admin-dashboard-scoping.md` when platform session opens.
2. **[PUSHED FROM switchable/social Session 1, 2026-05-02] Register Nina in the platform agent registry** alongside Thea, Iris, etc. Charlotte's directive: needed so any agent-attribution surface in the `/social` admin module recognises Nina when her drafts arrive post-Session-H.
3. **Verify tomorrow's 09:00 + 09:30 BST cron runs** (08:00 + 08:30 UTC). Both `meta-ads-ingest-daily` and `iris-daily-flags` should fire cleanly. Check `net._http_response` after 08:31 UTC for two 200s. If meta-ads-ingest 502s with "API access blocked", Meta verification gate has refired (see step 4).
