...
- **Riverside S4B Instant Form mapping FIXED this session:** free-text mapped to `additional_notes`, funding-use mapped to `interest`, ad id confirmed landing in `utm_content` (Make top-level `{{2.adId}}`). Multi-choice answers land clean (no array brackets). Backfilled historical employer leads: `additional_notes` (migration 0236, 5 rows) and `interest` (0237, 5 rows), values from Meta by owner, stored as option slugs. Duplicate lead 751 (Robert Atkins) marked as owner-test/archived. Note: 750 `interest='not_sure_yet'` is an inferred slug (only funding-use value not seen live), correctable.
- **Small platform display gap found (not fixed):** the admin lead-detail page renders `utm_source` but NOT `utm_content` (the ad id), so Instant Form ad ids are stored but not visible on the lead. Worth adding an "Ad ID" FieldRow. Ads-analytics attribution already uses `utm_content` correctly; it's dark only because the test campaign's spend hasn't synced to `meta_daily`.
- Hub tasks NOT filed (capture key permission denied this session).
