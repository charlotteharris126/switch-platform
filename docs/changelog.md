...
- Alan backfill: data-ops `052_replay_alan_immediate_impact_fastrack_ack_2026_08_03.sql` deleted his fastrack child row + nulled fastracked_at, then his original webhook body was replayed through the fixed EF. Result: `u_fastrack_qualified` email **sent** (alanashleyjones@gmail.com, 11:07 UTC); fastrack row restored (122, sole_trader, no mismatch); EMS sheet re-updated in place. The save-number **SMS did NOT fire** — Alan's LA is `york` and EMS `regional_contacts.by_la` has no York / North Yorkshire rep (only the seven Tees Valley / NE LAs), so `fireSaveNumberSms` skipped silently. Not a code bug; a provider-data gap affecting all three York & N.Yorks bootcamps (Immediate Impact, Digital Edge, Creative Catapult). Needs EMS to supply a York/N.Yorks rep name+phone, then `crm.providers.regional_contacts` update.
