...
- Deploy: `fastrack-receive` deployed via `supabase functions deploy`; admin + provider portal via switch-platform main (`a7dea47`, Netlify auto-deploy).
- Alan backfill: data-ops `052_replay_alan_immediate_impact_fastrack_ack_2026_08_03.sql` deleted his fastrack child row + nulled fastracked_at, then his original webhook body was replayed through the fixed EF. Result: `u_fastrack_qualified` email **sent** (alanashleyjones@gmail.com, 11:07 UTC); fastrack row restored (122, sole_trader, no mismatch); EMS sheet re-updated in place. The save-number **SMS did NOT fire** — Alan's LA is `york` and the York & N.Yorks bootcamps have no dedicated regional rep (by design), so `fireSaveNumberSms` had no number to send and skipped. Correct behaviour, not a bug, and no fix wanted: the qualify-ack email's named-rep/phone block also falls back cleanly (`renderProviderContactValues` no-match branch → generic "they'll be in touch in a few days" sentence, phone rendered invisibly). Both channels handle no-rep gracefully.
- Follow-up (separate, low priority): `composeFastrackNotes` in fastrack-receive omits `business_status_reconfirmed`, so the EMS sheet `fastrack_notes` summary doesn't carry the reconfirmed structure. Cosmetic (the portal now shows it); fold into the next fastrack-receive change.
- Signed off: Owner (session 2026-08-03).
...
- Change: additive nullable `leads.submissions.business_status TEXT`. Values `sole_trader | limited_company | partnership | not_trading`. Captured from the new `business_status` funded qualifier step on the three EMS York & North Yorkshire courses (digital-edge, immediate-impact, creative-catapult). Mirrors 0224 (support_role_sector) exactly.
- EF source threaded (additive, mirroring support_role_sector): `_shared/ingest.ts` (interface + `normalise()` parse of `business_status`/`business-status` + INSERT column + VALUES) and `_shared/route-lead.ts` (SubmissionRow interface + both SELECT lists + SUBMISSION_FULL_COLUMNS + `appendToProviderSheet` payload key).
- Fastrack reconfirm: `fastrack-receive/index.ts` + the thank-you page re-ask business status and DQ a disqualifying answer (mirrors the L3 / support-role reconfirms). The disqualifying answer is PER-COURSE, so the thank-you page computes the mismatch from `matrix.json` (`route.businessStatuses`, added to the matrix build) and posts `business_status_mismatch`; the EF trusts that flag, stores both fastrack columns, and flips the enrolment to Lost with `lost_reason='business_status_mismatch'`. No pay fork. CC (allows not_trading) never DQs; DE/II DQ not_trading.
- Portal + admin display (DONE this session): `business_status` renders on the admin lead detail (`app/admin/leads/[id]/page.tsx`, select `*`; also added the previously-missing earnings_band + support_role_sector rows) and the provider portal (`app/provider/leads/[id]/lead-detail-view.tsx` interface + Row + `BUSINESS_STATUS_LABELS`), with the column added to both explicit selects (`provider/leads/[id]/page.tsx` and `admin/preview/.../page.tsx`, the latter also missing earnings_band/support_role_sector, now fixed).
- Why: drives the 90/100% Skills Bootcamp funding split (sole traders 100%; limited companies 90% + 10% co-invest) the provider settles at enrolment. Without it the provider gets a lead with no business-structure signal. Source-of-truth spec: `switchable/site/docs/funded-funnel-architecture.md` (2026-07-30).
- Impact (§8): additive nullable column; no existing consumer reads it yet. Producer = funded form via netlify-lead-router. `readonly_analytics` auto-reads it (quasi-identifier, not PII). data-architecture.md updated (also backfilled the missing support_role_sector doc line). No schema_version bump (additive).
- **Deploy order (owner / correctly-connected session):** (1) apply migrations 0233, 0234, 0235 FIRST; (2) redeploy `netlify-lead-router`, `netlify-leads-reconcile`, `fastrack-receive`, and every `_shared/route-lead.ts` importer (the ~14 funcs listed in the 2026-06-15 private-pay entry); (3) deploy the admin app (lead-detail display); (4) the switchable/site deploy carries the thank-you page + matrix change. Production project ref = `igvlngouxcirqhlsrhga` (confirmed via read-only Postgres MCP: 7 providers, support_role_sector live, business_status absent). Fastrack page needs a real `?ref` browser pass post-deploy before the DQ flow is trusted.
- **BLOCKER:** the Supabase MCP in the authoring session was connected to the WRONG project (a bare project with only default schemas — no `leads`/`crm`, no migration history), so the migration could NOT be applied and the EFs could NOT be deployed from that session. Fix the MCP project target (or run `supabase db push` + `functions deploy` directly) before this ships. Nothing was applied to production.
- **Outstanding (not blocking capture):** (a) EMS York/N.Yorks regional caller — no `regional_contacts.by_la` entry for `york` / `north-yorkshire` yet, so York learners get the generic "who's calling" copy and no chaser SMS (same as south-tyneside today); add a data-ops `jsonb_set` once EMS give the caller name + phone. (b) Provider portal + admin lead-detail display of `business_status` (admin app change) so EMS can see sole-trader vs limited on each lead. (c) EMS Google Sheet: not wired as a sheet column (sheet is mid-teardown, portal-only going forward); value stays DB/portal.
- Signed off: Owner (session 2026-07-30) — pending deploy.
...
- **`_shared/meta-capi.ts` (new):** `sendCapiLead` (SHA-256 hashes em/ph/fn/ln/zp/country/external_id, rebuilds fbc from fbclid when cookie absent, POSTs Lead to graph.facebook.com, never throws) + `logCapiSend` (writes leads.capi_log). Token from `META_CAPI_ACCESS_TOKEN` (System User, never-expire), test mode via `META_CAPI_TEST_EVENT_CODE`, kill switch `META_CAPI_DISABLED`.
- **Routers wired:** `netlify-employer-lead-router` (B2B, pixel 1386293849929367, value 400, content_category employer_lead) fires on the routed path; `netlify-lead-router` + `_shared/ingest.ts` (B2C, pixel 1163964622558929, value funding_category='gov' 150 / else 100) fires for PRIMARY leads only (`parent_submission_id IS NULL` AND event_id present), matching the browser-pixel population so paid-lead counts stay honest.
- **Migration `0214_capi_reconcile_cron.sql` (NOT applied):** schedules `capi-reconcile-daily` (08:10 UTC). The function (new, verify_jwt=false) compares expected vs successfully-sent CAPI per brand over 25h and emails the owner on any gap or failed send. x-audit-key auth via Vault.
- **Deploy order (pending owner go-ahead + token):** set `META_CAPI_ACCESS_TOKEN` (+ optional `META_PIXEL_ID_B2C/B2B`) in EF secrets → apply 0213 → deploy `netlify-lead-router`, `netlify-employer-lead-router`, `capi-reconcile-daily` (and any other `_shared/ingest.ts` / `_shared/meta-capi.ts` importers) → apply 0214 → live Test Events check both brands → regenerate the token exposed in chat.
- Full plan + impact assessment: `platform/docs/capi-server-side-scoping-2026-06-15.md`.
- **Signed off:** owner approved the build (session 2026-06-15); deploy pending.
...
- **Follow-up (same session):** the live provider portal uses RLS (fixed by 0210) so it already showed private-pay leads, but the admin **preview** bypasses RLS with the admin client and applied its own `.not("is_dq","is",true)` filter (preview leads list + home dashboard), so private-pay leads were still invisible there. Changed both to `.or("is_dq.not.is.true,pay_route.eq.private")` to mirror the widened policy. Preview detail scopes by id + primary_routed_to (no is_dq filter), so click-through already worked.
- **Signed off:** owner (session 2026-06-15).
...
- **Provider context (`_shared/route-lead.ts`):** to stop a private-pay lead landing on the provider looking funded, the "self-funding, bill the learner directly" signal now rides the routed row itself: a `PRIVATE PAY — learner self-funds...` line prepended to the sheet `notes` column (status stays `Open` so reconcile is unaffected), a new `pay_route` field in the sheet-append payload, and a PII-free yellow callout in the provider notification email. Owner-confirm fallback (multi-candidate / non-auto-route providers) keeps its existing PRIVATE PAY banner.
- **Admin display (`app/admin/leads/page.tsx`, `app/admin/leads/[id]/page.tsx`):** private-pay leads (`pay_route='private'`) now show an amber "Private" / "Private pay" badge instead of the destructive "DQ" badge, and routed private-pay leads surface their enrolment status, U1 status, outcome form, notes, and activity like any normal routed lead. The underlying `is_dq` column is unchanged (`true` = "didn't get a funded place"); billing/analytics still correctly exclude them from funded counts. Only routing + display changed.
- **Site (separate repo `switchable/site/deploy`):** per-provider, per-course sign-off via a new `accepts_private: true` on the page-YAML provider entry. The private-pay offer + routing only fire when the course supports it (`private_option.enabled`) AND a provider on that page has signed off. Build narrows `provider_ids` on the private path to signed-off providers only. Set on EMS's two signed courses (Build an Online Shop £1,400 ex VAT, Intro to Management/Team Leading £599 ex VAT). New providers default off until onboarding records sign-off + price.
- **Deploy:** redeployed the 14 functions that bundle `_shared/route-lead.ts` (admin-brevo-resync, admin-notify-callback, admin-test-email, backfill-sw-provider-contact-block, brevo-attribute-reconcile, fastrack-receive, netlify-employer-lead-router, netlify-lead-router, republish-provider-sheet, routing-confirm, sheet-drift-reconcile-daily, sheet-edit-mirror, sms-chaser-attempt-1, sms-fastrack-prompt-cron). Admin app pages ship via Netlify on push.
- **Side effect (note):** that shared-bundle redeploy also shipped the per-course fastrack-flag fix committed 2026-06-14 (21e6f17), which was built+committed but deploy-pending. It's additive/corrective and matches the already-applied 0208, so the code is now consistent with the live DB. The waitlist-backfill panel + Kirsty resync from that workstream remain separate follow-ups.
- **Saranya (sub 639):** routes on the owner clicking the confirm button in the existing PRIVATE PAY email (token valid 14 days); now produces the corrected routed-to-EMS-with-bill-the-learner output. All future private-pay leads auto-route, no click.
- **Impact:** no reader breaks. `is_dq` semantics preserved. Zero routing change for existing single-provider pages (private `provider_ids` narrowing is a no-op there).
- **Signed off:** owner (session 2026-06-15).

## 2026-06-14 — Waitlist identity backfill function + fastrack-flag per-course fix (built, deploy pending owner go)
- Migration: `0208_backfill_waitlist_identity_fn.sql` — adds `crm.backfill_waitlist_identity_from_parent()` (SECURITY DEFINER, idempotent). Copies first_name/last_name/postcode/region/la/current_qualification onto opted-in `/waitlist/` child rows from their `parent_submission_id` row (NULL-fill only), then calls `crm.sync_leads_to_brevo` on the touched ids. Course interest NOT recovered (only 3/36 parents carry course_id; owner decision 2026-06-14 to leave existing 36 course-blank, no re-engagement email). Driven by a one-click panel at `/admin/data-ops/backfill-waitlist-identity` (Server Action `backfillWaitlistIdentityAction`, mirrors run-scheduled-publish). Context: `platform/docs/waitlist-capture-fix.md`. Data check before build: 44/45 waitlist rows have a parent; for all 36 opted-in, parent has name+location 36/36, qual 18/36.
- Code (no migration): `_shared/route-lead.ts` `loadEmailAggregateState` — `SW_FASTRACK_COMPLETED` changed from the single-canonical-submission flag to a per-canonical-course `bool_or(fastracked_at)`. Fixes the Amanda Robinson / Kirsty Crowther bug (S19 watch): a same-course re-application child (fastracked_at NULL) became canonical and wiped a real fastrack completion. Keeps Wren's 2026-05-25 cross-course fix (a fastrack on a different course still doesn't count). Live victims at diagnosis: 1 (Kirsty). Imported by `brevo-attribute-reconcile`; bundled by the route-lead writers.
- Deploy plan (PENDING owner go, batched): apply 0208; redeploy the fastrack-flag writers (`netlify-lead-router`, `routing-confirm`, `fastrack-receive`, `brevo-attribute-reconcile`, `admin-brevo-resync`, `netlify-employer-lead-router`); admin app (data-ops panel) ships via Netlify on push. Then run the waitlist backfill panel + resync Kirsty.
...
- EF: `_shared/ingest.ts` — persists `pay_route` (normalised to 'private' | NULL) and no longer clamps provider_ids to [] for a private DQ row (so it stays routable). `_shared/route-lead.ts` — `routeLead` selects `pay_route` and no longer rejects a private row at the `is_dq` guard. `netlify-lead-router` — private leads take the routing path despite is_dq, NEVER auto-route (forced to owner-confirm so the owner sees a PRIVATE PAY banner in the notification email before forwarding to the provider).
- Billing: a private enrolment is created and billed through the normal `crm.ensure_open_enrolment` path (funding_category stays gov, standard flat £150 EMS fee). The provider bills the learner the course fee; we bill the provider as usual.
- Deployed: `netlify-lead-router`, `routing-confirm` (owner-confirm calls routeLead on the private row), `netlify-leads-reconcile` (shares ingest.ts). Migration applied BEFORE the EF deploys (so no INSERT referenced a missing column). Verified: column live (text, nullable); zero private-pay leads landed in the gap between the site deploy and the EF wiring.
- Impact: no consumer of `leads.submissions` breaks (additive column). The other 9 `_shared/route-lead.ts` importers keep their pre-0207 bundled copy (the change is backward-compatible — they never see private leads); they can be redeployed in a routine sweep, no functional need.
- Follow-ups (NOT done, not blocking): provider-sheet "Pay Route" column + appender FIELD_MAP entry (so an auto-routed private lead would show it — moot while private is forced to owner-confirm); learner-portal "paying privately" badge. Fastrack trees A/B (spec `switchable/site/docs/private-pay-platform-spec.md` §5) is the larger follow-on; Mable builds the front-end once the `fastrack-receive` field contract is set.
- Spec: `switchable/site/docs/private-pay-platform-spec.md`. Sign-off: Charlotte (this session).

## 2026-06-11 — Data health clean slate: drift cards, severity-split digest, Brevo auto-resolve, backlog clear
- Migration: `0206_clear_routine_deadletter_backlog.sql` — one-off data fix. `UPDATE leads.dead_letter SET replayed_at = now()` on currently-unresolved rows for the routine/transient/test sources (sheet_drift_detected, brevo_attribute_reconcile_async_check_result, edge_function_partial_capture, edge_function_brevo_upsert, edge_function_labs_event, fastrack_form, netlify_audit, brevo_transactional_sms). Cleared 100 rows. No business row touched.
- App: `app/app/admin/errors/page.tsx` — added plain-English `SOURCE_EXPLANATIONS` for the six drift sources that previously fell through to the "Unknown ingestion error → Flag for Claude" default. They now read as info/clean with accurate "what to do".
- EF: `drift-digest-daily` — daily email now splits "Needs you" (real failures) from "Routine, no action" and leads with the count; a routine morning reads "All clear". `brevo-attribute-reconcile` — auto-resolves each source's prior dead_letter rows on every run so the Brevo summary logs stop accumulating (ticket e2b2615f); the sheet-drift cron already self-cleans.
- Context: dead_letter had ~114 unresolved, 109 routine. Root cause: the page only had plain-English cards for some sources, and the daily reconcile checks wrote their findings into the same table the dashboard reads. Investigated each "real bug": fastrack #1005 = a test submission (ref=test); netlify_audit #998 = transient (URL live, 200); labs_event ×2 = transient blip during the 0184/0185 deploy on 3 Jun (functions_writer inserts fine since — verified 4 events in labs.events_analytics 4-6 Jun); brevo_transactional_sms = Brevo SMS credits ran out (operational top-up). No outstanding code bugs.
- Deliberately not cleared by 0206: `brevo_attribute_drift` (14) — reconciled by an operator Brevo Re-sync (DB→Brevo) so the stale contacts are genuinely fixed, then auto-resolved by the redeployed cron; and `reconcile_backfill` (3) — today's recovered leads, verified handled (Olamide #604 routed+emailed; 2 waitlist rows don't route), pending owner acknowledgement on the dashboard.
- Impact (§8): app + EF + a row-only data fix. No schema change, no consumer reads affected beyond a shorter unresolved-dead_letter list. Rollback: revert the three code changes; 0206 has no rollback (routine drift re-detected by the daily crons if still present). Signed off: Owner (Charlotte, 2026-06-11 session).
...
- **Agent + /handoff write path = task-upsert EF** (not a DB function). Migration `0194` first added SECURITY DEFINER `add_work_task`/`update_work_task` granted to `readonly_analytics`, but testing showed the Postgres MCP wraps queries in a READ ONLY transaction that refuses writes even from a definer function ("cannot execute INSERT in a read-only transaction"). Migration `0197` drops both. Net: §11 "agents never write the DB" left fully intact. `task-upsert` EF extended to handle **update** (incl. tick-off via `status:'done'`) alongside insert; no delete (owner-only). `0199` grants `functions_writer` UPDATE on `strategy.tasks` (+ RLS policy) so the EF's update works.
- **Auth = Vault key, zero device storage** (migration `0198`). The earlier plan (a `TASK_UPSERT_SECRET` env var per device) was rejected: the workspace is iCloud-synced (secrets can't live there) and writing outside it is barred (harness-enforced). Instead `TASK_CAPTURE_KEY` is generated in-DB into the Vault; the EF validates against it; laptop-side callers fetch it at runtime via `public.get_task_capture_key()` (a narrow `SECURITY DEFINER` reader granted to `readonly_analytics` — returns only this one task-scoped key, not the delete-capable AUDIT key). Documented as the standard "Laptop-side automation auth" pattern in `platform/CLAUDE.md`. **No per-device setup, no activation step.**
- **Cutover wiring:** `/handoff` step 4 rewritten (ClickUp → fetch key via MCP → task-upsert EF) + step 5 files a Hub task on cross-project push; workspace `CLAUDE.md` ticketing rule + `/prime-project` step 4 + session-start line now point at the Hub. ClickUp Task pipeline + Backlog retired; Prospect + Client lead pipelines stay on ClickUp.
- **Verified end-to-end (2026-06-08):** read-only MCP fetches the key; EF add → ok, update/tick-off → ok, wrong key → 401. Task `94d8770c` is the verification record (Completed, auto-clears in 30d).
- Impact assessment (§8): writes only `strategy.tasks` (+ `leads.dead_letter` on EF failure); no PII; no schema_version bump; `readonly_analytics` gains read of one scoped key only; rollback = DOWN sections + cron.unschedule. Signed off: Owner (2026-06-08, "go" + "lets just sort it").
...
- **Verified safe (no path broken):** callback flagging only clears the two granted callback cols (raising is admin/service-role); sheet/webhook EFs (`sheet-edit-mirror`, `crm-webhook-receiver`, `reconcile-sheet-to-db`) write enrolments over the EF DB connection (`SUPABASE_DB_URL` = postgres superuser; `reconcile-sheet-to-db` additionally `SET LOCAL ROLE functions_writer`), none as the provider `authenticated` role; admin billing writes via service role (not subject to column grants); every enrolment-mutating DB function is `SECURITY DEFINER` (runs as owner, bypasses caller grants) — no `SECURITY INVOKER` function updates the table.
- **Wording correction (2026-06-02):** the original entry + 0180 header said sheet sync writes "via functions_writer"; accurate per Codex recheck is the above (only `reconcile-sheet-to-db` drops to functions_writer; the other two write as postgres on the EF connection). Conclusion unchanged — none use the provider role, none affected. 0180 header comment amended (comment-only, no executable change).
- **Residual least-privilege note (low risk):** granting `authenticated` UPDATE on `callback_requested_at`/`callback_requested_by` lets a direct API caller SET, not just clear, those values. No billing/commercial impact. If stricter least privilege is wanted later, move callback clearing to an RPC / service-role action and drop those two from the provider column grant.
...
## 2026-05-31 — Brevo drift root causes: whitespace false-positive (fixed) + missing Category enum values (owner)

Re-syncing wasn't dropping the drift count (110→142, not →0). Investigated the persistent attributes. Three distinct causes, only one was a code bug:

- **Whitespace false positive (CODE FIX, pending deploy) — ~115 of the drift.** `normaliseForCompare` in `brevo-attribute-reconcile` returned strings un-trimmed despite its own comment saying "trimmed". Brevo trims leading/trailing whitespace when it STORES an attribute; the canonical projection pushes raw DB values. DB carries trailing spaces on many names ("Obinna ", "Chloe ") and a deliberate leading space on phones (" 0782…" — the Netlify numeric-coercion guard). So desired " 0782…" vs Brevo-stored "0782…" flagged as permanent drift no re-apply could fix. Fixed: trim both sides in the comparison (whitespace-only diffs collapse; real value changes still drift). Covers SW_PHONE 67 + FIRSTNAME 27 + LASTNAME 21. Deploy: `supabase functions deploy brevo-attribute-reconcile --no-verify-jwt`.
- **SW_ENROL_STATUS (52) — OWNER, Brevo dashboard, no code fix.** The Brevo `SW_ENROL_STATUS` Category attribute enum is missing values the builder now pushes. Brevo silently drops writes to undefined Category values, so the contact keeps its old status and drift is reported forever. The status set grew (attempt_* + enrolment_meeting_booked + employer statuses added to admin UI ~S60) but the Brevo enum wasn't extended. **Fix: Brevo → Contacts → Settings → Attributes → SW_ENROL_STATUS, add:** attempt_1_no_answer, attempt_2_no_answer, attempt_3_no_answer, enrolment_meeting_booked, engaged, in_progress, signed, not_signed, presumed_employer_signed. (cannot_contact, open, enrolled, presumed_enrolled, lost already exist.) Then re-run apply.
- **SW_COURSE_OPEN (16) — test pollution, ignore/delete.** All 16 are owner-test leads (hello+…@switchable / kieranwrites@, is_dq owner_test, routed to Riverside) sitting in Brevo. Correctly flagged, irrelevant. Delete those test contacts from Brevo or ignore.
- **Errors 60→13 confirms the gate fix is live** — only demo-provider-ltd (archived) leads error now, which is correct.
