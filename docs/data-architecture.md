...
- Regional contact mapping (added 0145): `regional_contacts JSONB`, by-LA lookup keyed by `leads.submissions.la`. Resolved server-side in `_shared/route-lead.ts:renderProviderContactBlock` and pushed as the Brevo contact attribute `SW_PROVIDER_CONTACT_BLOCK` on every Switchable learner upsert (matched + no_match + pending). Regional-match leads (EMS today) get a named-rep + phone paragraph; everyone else gets a unified fallback paragraph. NULL for every provider without regional reps; EMS is the only regional-match consumer at v1. Shape: `{ "by_la": { "<la-slug>": { "first_name": "...", "name": "...", "phone": "..." } } }`. Wiring switched from per-send param to contact attribute on 2026-05-16 (Session 48) for Brevo template preview visibility and consistency with the rest of the SW_* attribute set.
