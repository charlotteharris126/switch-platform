...
- Raw `labs.events` (incl `email`) is **not** exposed to the data API and is **not** readable by the reporting role. It's read operationally only: the owner via the admin app through SECURITY DEFINER, service-role-only RPCs `public.admin_labs_funnel()` + `public.admin_labs_recent_signups()` (0183), behind the admin login. The `labs` schema is deliberately kept off the public data API, so the email table has no API surface at all; the only read path is the service-role RPCs. There is intentionally **no** `authenticated`/admin RLS read policy or grant on the base table — RLS deny-by-default means a future accidental schema exposure fails closed. (0184 briefly added a parity policy; 0185 dropped it as inert and misleading. If `labs` is ever intentionally API-exposed, the admin read policy + matching grants get added together at that point.)
- `labs.events_analytics` (view, 0184) is the direct-identifier-free projection of the table — every column except `email`. This is what `readonly_analytics` (agents over the MCP) reads; it has no SELECT on the base table. Direct-identifier-free, not anonymous: session id, attribution, referrer and user-agent remain because they're the analytical substance.
...
- Regional contact mapping (added 0145): `regional_contacts JSONB`, by-LA lookup keyed by `leads.submissions.la`. Resolved server-side in `_shared/route-lead.ts:renderProviderContactValues` and pushed as three plain-text Brevo contact attributes on every Switchable learner upsert (matched + no_match + pending): `SW_PROVIDER_CONTACT_BEFORE`, `SW_PROVIDER_PHONE`, `SW_PROVIDER_CONTACT_AFTER`. The U1 funded template wraps them as `<p>{{contact.SW_PROVIDER_CONTACT_BEFORE}} <strong>{{contact.SW_PROVIDER_PHONE}}</strong> {{contact.SW_PROVIDER_CONTACT_AFTER}}</p>` — HTML wrapper in the template because Brevo's text-type attributes always escape-render. Regional-match leads (EMS today) get a named-rep + phone split; everyone else gets a unified fallback sentence in `BEFORE` with `PHONE`/`AFTER` empty (template's empty `<strong></strong>` renders invisibly). NULL for every provider without regional reps; EMS is the only regional-match consumer at v1. Shape: `{ "by_la": { "<la-slug>": { "first_name": "...", "name": "...", "phone": "..." } } }`. Redesign history: shipped 2026-05-15 (Session 47) as a per-send transactional param `SW_PROVIDER_CONTACT_BLOCK`, switched to a single contact attribute 2026-05-16 morning (preview visibility), then split into three plain-text attributes 2026-05-16 afternoon when Brevo's text-attribute escape behaviour proved un-defeatable on the template side. The original `SW_PROVIDER_CONTACT_BLOCK` attribute is orphaned and pending deletion from Brevo.
...
CREATE TABLE leads.submissions (
  ...
  -- Referral programme (added in migration 0053). referral_code is unique,
  -- auto-generated by a BEFORE INSERT trigger if not supplied, backfilled for all
  -- pre-existing rows. referrer_lead_id is set when this lead arrived via
  -- ?ref=CODE on a Switchable URL. NULL for organic / paid social.
...
One row per referred lead. Created when a submission arrives with a valid `?ref=CODE`. Tracks the lifecycle from `pending` (just created, awaiting friend's enrolment) through `eligible` (friend enrolled, voucher ready to fire) to `paid` (Tremendous payout completed). `fraud_rejected` is terminal and set at submission time when an anti-fraud check fails.
...
CREATE TABLE leads.referrals (
  ...
  voucher_amount_pence  INT NOT NULL DEFAULT 5000,  -- £50, set per-row so it can change without migration
  needs_manual_review   BOOLEAN NOT NULL DEFAULT false,
  fraud_reason          TEXT,
  notes                 TEXT,

  -- Vendor (Tremendous in v1)
  vendor                TEXT,
  ...
  CONSTRAINT leads_referrals_no_self CHECK (referrer_lead_id <> referred_lead_id)
);

CREATE INDEX leads_referrals_referrer_idx ON leads.referrals (referrer_lead_id, voucher_status);
CREATE INDEX leads_referrals_status_idx ON leads.referrals (voucher_status, eligible_at)
  WHERE voucher_status IN ('pending', 'eligible');
CREATE INDEX leads_referrals_review_idx ON leads.referrals (needs_manual_review, eligible_at)
...
|---|---|---|---|
| (insert) | `pending` | `netlify-lead-router` when `?ref=CODE` resolves to a referrer and anti-fraud passes | Row created |
| (insert) | `fraud_rejected` | Same, when anti-fraud fails (self-referral by email/phone/address, or referred email already in `leads.submissions`) | `fraud_reason` set, no voucher |
| `pending` | `eligible` | `leads.flip_referral_eligible(submission_id)` from the enrolment-confirmation path (confirmed enrolment OR 14-day presumed-enrolment auto-flip) | `eligible_at = now()`, `needs_manual_review` set if soft cap hit |
| `eligible` | `paid` | `payout-referral-voucher` Edge Function on successful Tremendous API call | `vendor_payment_id`, `vendor_payload`, `voucher_paid_at` populated |

**Anti-fraud (enforced in Edge Function, NOT migration):**
- Self-referral block: same email, phone, or address as referrer
- Duplicate-email block: friend's email already exists in `leads.submissions`
- Soft cap: 10 successful referrals per referrer per 90 days flags `needs_manual_review = true` instead of blocking; voucher does not auto-fire until owner clears

**Writers:**
- `netlify-lead-router` (extended in a follow-up Edge Function patch) — INSERT at submission time
- `crm` enrolment-confirmation path — calls `leads.flip_referral_eligible(submission_id)`
- `payout-referral-voucher` Edge Function (new) — UPDATE to status=paid after Tremendous call
- Admin dashboard via `admin_update_referrals` policy — manual review clearance, notes

**Readers:**
- `/admin/actions` (referrals needing manual review)
- `/admin/referrals` (full list, planned)
- Mira KPI scorecard
- `payout-referral-voucher` Edge Function (polls eligible rows where `needs_manual_review = false`)

**Helpers:**
- `leads.generate_referral_code()` — returns an 8-char Crockford base32 string. Called by the BEFORE INSERT trigger on `leads.submissions` if `referral_code IS NULL`.
- `leads.flip_referral_eligible(submission_id BIGINT) RETURNS BOOLEAN` — finds the pending referral for the given referred lead, flips to `eligible`, sets `needs_manual_review` if the soft cap is hit. Idempotent.
