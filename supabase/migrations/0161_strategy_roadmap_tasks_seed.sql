...
('/admin/roadmap schema migration (0160) written', 'platform/supabase/migrations/0160_strategy_roadmap_tasks.sql — creates schema, table, 3 indexes, 1 trigger, 2 RLS policies. File shipped.', 'foundation', 'p1', '{sasha}', 'complete', '2026-05-23 17:45:00+00', 60),
('/admin/roadmap seed migration (0161) written', 'platform/supabase/migrations/0161_strategy_roadmap_tasks_seed.sql — this migration. ~99 tasks pre-categorised across all 10 revenue models. File shipped.', 'foundation', 'p1', '{sasha,mira}', 'complete', '2026-05-23 18:00:00+00', 70),
...
('/admin/roadmap Edge Function endpoints (admin-roadmap index.ts + config.toml)', 'Single Edge Function with action-discriminated CRUD (list/update/create). x-audit-key auth matching existing /admin/* pattern. config.toml updated with verify_jwt=false. Files shipped.', 'foundation', 'p1', '{sasha}', 'complete', '2026-05-23 18:15:00+00', 90),
('/admin/roadmap frontend admin page (actions.ts + page.tsx + roadmap-client.tsx)', 'Server Actions wrapping the Edge Function with isAdmin gate. Server Component for auth + initial load. Client Component with filters (revenue model / phase / status / hide-complete), grouped by revenue model, status dropdown inline, notes textarea with 500ms debounced auto-save, completion strikethrough, saving/saved feedback. Files shipped.', 'foundation', 'p1', '{sasha}', 'complete', '2026-05-23 18:30:00+00', 100),
('/admin/roadmap sidebar entry added (admin-shell.tsx)', 'Roadmap link added to Tools section of the platform admin sidebar so it''s discoverable alongside Analytics, Experiments, Automations, etc. File shipped.', 'foundation', 'p1', '{sasha}', 'complete', '2026-05-23 18:35:00+00', 105),
('Deploy /admin/roadmap to production (supabase db push + supabase functions deploy + git push for Next.js page)', 'Three commands from platform folder. Migrations apply schema + seed. Function deploy via supabase functions deploy admin-roadmap --no-verify-jwt. Next.js page auto-deploys via Netlify on push to main.', 'foundation', 'p1', '{charlotte}', 'to_do', NULL, 108),
