...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  if (refCode) {
    ...
    const refTask = processReferral(sql, result.id, refCode, row).catch((err) =>
...
// Referral programme processing (extractRefCode + processReferral) lives in
// _shared/referral.ts so netlify-leads-reconcile can apply identical anti-fraud
// + row-insert logic when back-filling missed leads.
